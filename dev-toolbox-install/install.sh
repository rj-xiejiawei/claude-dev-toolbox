#!/bin/bash
# 开发工具箱一键安装脚本
# 包含: Claude CLI, Lark CLI, Oh My Zsh, nvm, SDKMAN, Java, Go等完整工具链
...
if [ "$INSTALL_OH_MY_ZSH" = true ]; then
  if [ ! -d "$HOME/.oh-my-zsh" ]; then
    ...
    # 更新.zshrc插件配置
    if [ -f ~/.zshrc ]; then
      sed -i 's/^plugins=(git)/plugins=(git zsh-autosuggestions fzf-tab)/' ~/.zshrc
      log "✅ Zsh插件配置已更新"
    fi
  else
    log "✅ Oh My Zsh 已安装"
